...
describe("validatePurchaseForm", () => {
  it("returns null when all required fields are present", () => {
    expect(validatePurchaseForm({ supplier_id: 1, item_price: "50", amount: "10" })).toBeNull();
  });

  it("requires supplier_id", () => {
    const errors = validatePurchaseForm({ supplier_id: null, item_price: "50", amount: "10" });
    expect(errors).toEqual({ supplier_id: msg.blank });
  });

  it("requires item_price", () => {
    const errors = validatePurchaseForm({ supplier_id: 1, item_price: "", amount: "10" });
    expect(errors).toEqual({ item_price: msg.blank });
  });

  it("requires amount", () => {
    const errors = validatePurchaseForm({ supplier_id: 1, item_price: "50", amount: "" });
    expect(errors).toEqual({ amount: msg.blank });
  });

  it("reports all missing fields at once", () => {
    const errors = validatePurchaseForm({ supplier_id: null, item_price: "", amount: "" });
