...
export default function SearchBar({ initialQuery, path }: SearchBarProps) {
  const [query, setQuery] = useState(initialQuery);

  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    router.get(path, { q: query || undefined }, { only: ["customers", "pagination", "search"], preserveState: true });
  }
