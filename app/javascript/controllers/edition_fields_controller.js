...
export default class extends Controller {
  ...
  connect() {
    // Parse existing editions from JSON value
    if (this.hasExistingValue) {
      this.existingEditions = this.existingValue
    } else {
      this.existingEditions = []
    }

    // Store the original title on connect
    if (this.hasTitleTarget) {
      this.originalTitle = this.titleTarget.textContent.trim()
      // Don't show arrow on initial load if it's a new edition
      if (this.originalTitle !== "New Edition") {
  ...
  checkDuplicate() {
    ...
    // Get the ID of the current edition (for editing existing editions)
    const editionIdInput = this.element.querySelector('[name$="[id]"]')
    const currentEditionId = editionIdInput?.value

    const isDuplicate = this.existingEditions.some(edition => {
      // Skip comparing with itself when editing
      if (currentEditionId && edition.id == currentEditionId) return false

      const matchSize = (!sizeId && !edition.size_id) || edition.size_id == sizeId
      const matchVersion = (!versionId && !edition.version_id) || edition.version_id == versionId
      const matchColor = (!colorId && !edition.color_id) || edition.color_id == colorId
