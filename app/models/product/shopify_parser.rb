...
class Product
  class ShopifyParser
    ...
    # Example of a Shopify product title:
    # "Elden Ring - Malenia | 1:4 | Resin Statue | by Prime 1 Studio"
    def parse_shopify_title
      prepared_title = smart_titleize(sanitize(payload["title"]))
      parts = prepared_title.split("|").map(&:strip)

      # Parse franchise name and product title
      if parts[0].include?(" - ")
        franchise, product_name = parts[0].split(" - ")
      else
        # Sometimes (old products?) it's the same thing
        franchise = product_name = parts[0]
      end

      size = Size.parse_size(parts[1]) if parts[1]
      shape = parts[1]&.match(/Resin\s+(Statue|Bust)/i)&.[](1) || "Statue"

      # Brand is typically the last part if more than 2 parts exist
    ...
    # Shopify store SKUs in variants
    # The first variant is always present by default
    def parse_sku
      @parsed_sku = payload.dig("variants", "edges", 0, "node", "sku") || generate_sku
    end

    # Generate a "unique" SKU from the title
    def generate_sku
      base_sku = payload["title"].parameterize
      "#{base_sku}-#{Random.alphanumeric(4)}"
    end
