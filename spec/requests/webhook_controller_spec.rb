...
RSpec.describe WebhookController do
  ...
  describe "updates sales on /post" do
    before "mock a sale" do
    ...
    it "updates a product sale related to the parsed product" do
      ...
      expect(updated_product_sale.qty).to eq(1)
      expect(updated_product_sale.price).to eq(BigDecimal("235"))
      expect(updated_product_sale.variation.title).to eq("Deluxe Version")
    end
  end
