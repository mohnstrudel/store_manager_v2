...
RSpec.describe Shopify::PushMediaJob do
  describe "#perform" do
    ...
    before do
      ...
      allow(api_client).to receive(:push_media).with(shopify_product_id, kind_of(Array)).and_return(created_shopify_media)
    ...
    it "returns early if product has no media" do
      ...
      expect(api_client).not_to have_received(:push_media)
    end
    ...
    it "filters out media already synced to Shopify" do
      ...
      allow(api_client).to receive(:push_media).with(shopify_product_id, kind_of(Array)).and_return(
        [
          {"id" => "gid://shopify/MediaImage/456", "alt" => "Image 1", "mediaContentType" => "IMAGE", "status" => "UPLOADED"},
          {"id" => "gid://shopify/MediaImage/457", "alt" => "Image 2", "mediaContentType" => "IMAGE", "status" => "UPLOADED"}
        ]
      ...
      expect(api_client).to have_received(:push_media) do |_product_id, media_input|
        expect(media_input.size).to eq(2)
        expect(media_input.map { |i| i[:alt] }).to eq(["Image 1", "Image 2"])
      end
    end
    ...
    it "uploads unsynced media with correct data structure" do
      ...
      expect(api_client).to have_received(:push_media).with(shopify_product_id, kind_of(Array)) do |_id, media_input|
    ...
    it "calls reorder_media_on_shopify after upload" do
      # Mark all media as synced for reorder (push_media will create the store_infos)
      described_class.perform_now(shopify_product_id, product_id)
