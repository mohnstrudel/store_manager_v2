...
Use this file for the non-obvious request, controller, and backend presentation rules in this repo. For React and browser-side UI work, use the frontend architecture guide instead.
...
- Inline interaction flows such as edit or cancel or update can still be resourceful; a small singular nested controller is often cleaner than `edit_foo`, `cancel_foo`, and `update_foo` actions on the parent controller.
- Collection workflows can be resourceful too when the concept lives at the collection or dashboard boundary.
- After route extraction, update helper code and shared UI primitives to use the new route contract. Generic helpers should prefer route helpers or correctly shaped polymorphic calls with the right HTTP verb, rather than assuming the old path shape still works.
...
- presentation-only branching -> helpers, partials, Jbuilder, or response templates
...
- collection-level side effect or endpoint -> singular collection resource controller under the owning namespace
...
- If a backend template starts assembling a small collection of view data for one widget or partial, prefer a helper before inventing a presenter.
...
- If a screen starts to depend on client-side state or complex browser behavior, move that interaction design to the frontend guide instead of stretching backend presentation rules.
...
- Do not make browser-side interaction code fight backend responses or split one widget's lifecycle across multiple modules without a clear boundary.
...
- Follow the existing backend view patterns in this repo.
