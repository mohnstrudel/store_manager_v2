...
Design Rails apps around domain ownership and request-scoped boundaries, not around a large services layer.
...
- Introduce service objects only when coordinating multiple aggregates, external APIs, files, or workflow steps that do not belong to one model capability.
...
- Use association proxy extensions when a collection has its own commands, such as `grant_to`, `revoke_from`, or `revise`, instead of inventing a separate service object for collection-local behavior.
...
- Do not flatten association-proxy APIs into manager services if the behavior belongs to one relationship.
...
- Do not turn thin jobs into service-heavy workflow containers; keep the job as transport unless the workflow truly belongs there.
- Do not collapse rich capability tests into generic service tests when the model already owns the concept.
...
7. Introduce query objects or services only when subsystem complexity genuinely requires them.
...
- Do not “modernize” a model-centric design into a service-heavy one just because generic Rails advice says to.
