...
8. Write tickets to `<spec-directory>/tickets/<NN>-<slug>.md` using the adaptive contract. Audit the complete set for omitted decisions, duplicate ownership, hidden dependencies, oversized tickets, speculative defensive bloat, comment requests that code should express, and missing test-first cases before finishing.
