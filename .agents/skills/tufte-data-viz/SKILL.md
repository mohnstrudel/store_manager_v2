...
description: >-
  ...
  In this repo's React/Inertia frontend, prefer React/TypeScript examples
  and React wrappers over imperative vanilla snippets when a library
  supports both.
...
## React-first app notes

This repo's frontend is React 19 + Inertia + TypeScript in `app/frontend`.

When adapting chart guidance to this app:
- Prefer TSX components and hook-driven composition over imperative chart bootstrapping.
- If a charting library has a React wrapper, show that wrapper first. For Chart.js, prefer `react-chartjs-2` over raw `new Chart(...)` setup unless the user explicitly asks for imperative code.
- Keep chart components page-local and consistent with the app's existing frontend conventions.
- Translate any non-React example from a rule file into React/TypeScript before using it here.
...
| Chart.js | `rules/chartjs.md` | JavaScript. In this app, prefer the React wrapper first (for example, `react-chartjs-2`) and only fall back to imperative Chart.js setup when explicitly requested.
