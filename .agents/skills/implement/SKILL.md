...
7. Run the ticket's Focused verification plus checks required by its actual changes. Check off acceptance criteria after verification and set the ticket to `done`.

## Rules

- When the selected spec has no tickets, implement it as one unit and apply `AGENTS.md`'s risk-based completion policy once its required checks pass.
