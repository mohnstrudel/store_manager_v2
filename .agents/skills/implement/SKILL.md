...
7. Run only the ticket's Focused verification plus checks directly relevant to its actual changes — never the full `AGENTS.md` gate, another ticket's checks, or a check that already passed unchanged. Check off acceptance criteria only after verification, set the ticket to `done`, then make one commit for the ticket's changes using the `commit` skill's format.
8. Report the next frontier. When every ticket is `done`, run the full `AGENTS.md` verification gate if this is the coordinating task; otherwise report that requirement to the coordinating task and stop.
