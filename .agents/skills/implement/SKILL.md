...
8. Report the next frontier. When every ticket is `done` (or, for an unticketed spec, once its single unit is `done`), delete the entire spec iteration directory (`spec.md`, `tickets/`, and any other files under that `YYYYMMDDTHHMMSSZ__<spec-slug>` directory) in a dedicated commit before reporting completion — git history retains the content. The coordinating task then applies `AGENTS.md`'s risk-based completion policy to the combined change; other tasks report the frontier and stop.
...
- Once a spec iteration is fully done, remove it: do not leave completed `spec.md` or `tickets/*.md` files in the working tree as historical clutter.
